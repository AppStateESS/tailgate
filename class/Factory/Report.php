...
require_once PHPWS_SOURCE_DIR . 'mod/tailgate/vendor/autoload.php';
...
class Report
{
    ...
    public static function pickup($game_id)
    {
        ...
        $c1 = $db->createConditional($lottery->getField('spot_id'),
                $spot->getField('id'), '=');
        $db->joinResources($lottery, $spot, $c1, 'left');

        $lot = $db->addTable('tg_lot');
        $lot->addField('title');
        $c2 = $db->createConditional($spot->getField('lot_id'),
                $lot->getField('id'), '=');
        ...
        $c3 = $db->createConditional($lottery->getField('student_id'),
                $student->getField('id'), '=');
        $db->joinResources($lottery, $student, $c3, 'left');

        $users = $db->addTable('users');
        $users->addField('username');
        $c4 = $db->createConditional($student->getField('user_id'),
                $users->getField('id'), '=');
    ...
    public static function winners($game_id)
    {
        ...
        $c5 = $db->createConditional($lottery->getField('spot_id'),
                $spot->getField('id'), '=');
        $db->joinResources($lottery, $spot, $c5, 'left');

        $lot = $db->addTable('tg_lot');
        $lot->addField('title', 'lot_title');
        $c6 = $db->createConditional($spot->getField('lot_id'),
                $lot->getField('id'), '=');
    ...
    public static function spotReport($game_id)
    {
        ...
        $c1 = $db->createConditional($spot->getField('lot_id'),
                $lot->getField('id'), '=');
        $db->joinResources($spot, $lot, $c1);

        $lottery = $db->addTable('tg_lottery');
        $lottery->addField('id', 'lottery_id');
        $c2a = $db->createConditional($spot->getField('id'),
                $lottery->getField('spot_id'), '=');
        ...
        $student->addField('first_name');
        $student->addField('last_name');
        $c3a = $db->createConditional($lottery->getField('student_id'),
                $student->getField('id'), '=');
    ...
    private static function fillTitle($template, $game_id)
    {
        ...
        $template->add('kickoff',
                strftime(TAILGATE_DATE_FORMAT, $game->getKickoff()));
    }
