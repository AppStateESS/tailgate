...
class ConfirmSpot extends React.Component {
  ...
  loadAvailableSpots() {
    $.getJSON('tailgate/User/Lottery', {command: 'spotChoice'}).done(
      function (data) {
  ...
  confirmSpot() {
    const spotId = this.state.spotChoice
    if (spotId === '0') {
      return
    }
    const lotteryId = this.props.lottery.id
    this.setState({waiting: 1})
    $.post(
      'tailgate/User/Lottery',
      {
        command: 'pickSpot',
        spotId: spotId,
        lotteryId: lotteryId,
  ...
  render() {
    ...
    if (this.state.availableSpots.length > 0) {
      const options = this.state.availableSpots.map(function (val, i) {
        if (val.sober === '1') {
          sober = '(Non-alcoholic)'
        } else {
          sober = ''
      ...
      return (
        <div className="row">
          <div className="col-sm-12">
            <p>
              You have confirmed your winning ticket. Pick the spot you wish to
              tailgate in below. Choose quickly, other winners may be picking
              spots while you decide.
            </p>
            {this.state.message}
          </div>
          ...
          <div className="col-sm-4">
            <button className="btn btn-primary" onClick={this.confirmSpot}>
